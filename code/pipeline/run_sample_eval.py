...
def _process_claim(
    ...
    Returns (output_row, usage_dict). Raises RowFailure with a human
    readable message on any failure (image load problems are tolerated
    individually by load_images_for_claim/ImageRecord.error and surfaced to
    the model rather than raised here; this only raises for API/validation
    failures that should mark the whole row as failed).
    """
